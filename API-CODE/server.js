...
app.patch("/api/:post_id/add-comment", (req, res) => {
  const cookies = Cookies(req, res);
  ...
  User.findOne({username: cookies.get("username")})
  .then((result) => {
    const comment = Comment({text, author: new ObjectId(result._id), post_id});
    comment.save()
    .then((result) => {return res.status(200).json({message: result})})
    .catch((error) => {console.log(error); return res.status(400).json({message: "Ошибка при добавлении комментария!"})});
  })
  .catch((error) => {console.log(error); return res.status(400).json({message: "Ошибка при поиске пользователя!"})});
...
app.delete("/api/post-delete", (req, res) => {
    ...
    Post.findByIdAndUpdate(new ObjectId(post_id), {is_deleted: true})
    .orFail()
    .then(() => {return res.status(200).json({message: `Успешно удалено в ${new Date()}!`})})
